...
<cfelse>
	<cfinvoke component="alpine-objects.product" method="search" criteria="#attributes.criteria#" maxrows="25" returnvariable="products" resulttype="simple" domain="category" withproducts="false">

</cfif>

<cfset searchcategories = products.categories>

<cfoutput>#HTMLEditFormat(attributes.callBack)#({recordCount:#searchcategories.recordcount + 1#, records:[{n:'Top',i:'0',t:'c'}<cfloop query="searchcategories">,{n:'#replacelist(xmlformat(category),"&,/",",")#',i:'#categoryID#',t:'c'}</cfloop>]})</cfoutput>
